$.ajax({
	...
	$.each(result.results, function (key, val) {
		text +=
			`<tr>
			<td class="text-center text-capitalize">${key + 1}</td>
			<td class="text-center text-capitalize">${val.name}</td>
			<td class="text-center text-capitalize">
				<button type="button" onclick="detailPoke('${val.url}')" class="btn btn-primary" data-toggle="modal" data-target="#detailsModal">
  					Details
				</button></td>
		</tr>`;
	});
	console.log(result.results);
	$("#pokemonData").html(text);
});
